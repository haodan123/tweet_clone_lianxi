  ...
  <div>
    <div class="flex items-center flex-shrink-0 p-4 pb-0">
      ...
      <div class="flex w-12 items-top">
        <img :src="user?.profileImage" class=" inline-block w-10 h-10 rounded-full" alt="">
      </div>
    ...
    <!-- 默认隐藏这个文件选择器  点击下方的发送图片显示选择文件 -->
    <div class="p-4 pl-16">
      <!-- 回显的图片 -->
      <img :class="twitterBorderColor" class="border rounded-2xl" :src="inputImageUrl" v-if="inputImageUrl" alt="">

      <input @change="handleImageChange" type="file" accept="image/png,image/gif,image/jpeg" ref="imageInput" hidden>
    </div>
    ...
    <div class="flex p-2 pl-14">
      ...
      <div class="flex w-full text-white">
        <div @click="handleImageClick"
          class=" cursor-pointer p-2 text-blue-400 rounded-full hover:bg-blue-50 dark:hover:bg-dim-800">
          <!-- 选择图片的svg -->
        ...
        <div class=" cursor-pointer p-2 text-blue-400 rounded-full hover:bg-blue-50 dark:hover:bg-dim-800">
          <!-- GIF的svg -->
          <svg viewBox="0 0 24 24" class="w-5 h-5" fill="currentColor">
            <g>
              <path
      ...
      <!-- 右边的按钮 -->
      <div class="ml-auto">
        <UIButton :disabled="isDisabled" size="sm" @onClick ="handleFormSubmit">Tweet</UIButton>
...
<script setup>
  ...
  const text = ref('') //输入的评论
  const imageInput = ref() //获取文件选择的DOM
  const selectedFile = ref() //选中的图片
  const inputImageUrl = ref('') //回显的图片路径
  // 发送消息按钮是否可以使用
